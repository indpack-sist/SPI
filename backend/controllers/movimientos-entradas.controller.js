...
import { generarControlMateriaPrimaXLSX } from '../utils/excelGenerators/controlMateriaPrimaXLSX.js';
...
export const generarControlMateriaPrimaPDFController = async (req, res, next) => {
  try {
    ...
    const movimientos = await obtenerMovimientosMateriaPrima({ fecha_inicio, fecha_fin, id_tipo_inventario });
...
export const generarControlMateriaPrimaXLSXController = async (req, res, next) => {
  try {
    const { fecha_inicio, fecha_fin, id_tipo_inventario } = req.query;
    ...
    const movimientos = await obtenerMovimientosMateriaPrima({ fecha_inicio, fecha_fin, id_tipo_inventario });
    const xlsxBuffer = await generarControlMateriaPrimaXLSX(movimientos, { fecha_inicio, fecha_fin, id_tipo_inventario });

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="control-materia-prima-${fecha_inicio}_${fecha_fin}.xlsx"`);
    res.send(xlsxBuffer);
  } catch (error) {
    next(error);
  }
