...
function DetalleCompra() {
  ...
  const handleRegistrarIngreso = async (e) => {
    ...
    if (productosAIngresar.length === 0) {
      setError('Debe ingresar al menos un producto con cantidad mayor a 0'); return;
    }
    ...
    if (productosExcedidos.length > 0) {
      setError('Algunas cantidades exceden lo pendiente'); return;
    }
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
        ...
        <div className="card mb-6 border-l-4 border-purple-500 bg-purple-50">
          <div className="card-body">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="font-bold text-purple-900 flex items-center gap-2"><RefreshCw size={18} /> Reembolso Pendiente</h3>
                <p className="text-sm text-purple-700 mt-1">Comprador: <span className="font-medium">{compra.comprador || 'No especificado'}</span></p>
                <p className="text-sm text-purple-700">Estado: <span className={`badge ${compra.estado_reembolso === 'Pendiente' ? 'badge-danger' : compra.estado_reembolso === 'Parcial' ? 'badge-warning' : 'badge-success'}`}>{compra.estado_reembolso}</span></p>
              </div>
              <div className="text-right">
                ...
                <button className="btn btn-sm btn-purple text-white mt-2" onClick={handleAbrirReembolso}><DollarSign size={16} /> Reembolsar</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mb-6">
        <div className="border-b border-gray-200">
          <nav className="flex gap-4">
            ...
            <button className={`px-4 py-2 border-b-2 font-medium transition ${tabActiva === 'ingresos' ? 'border-primary text-primary' : 'border-transparent text-muted hover:text-gray-700'}`} onClick={() => setTabActiva('ingresos')}>
              Ingresos Inventario {itemsPendientes.length > 0 && <span className="ml-2 badge badge-info text-xs">{itemsPendientes.length}</span>}
            </button>
          </nav>
        </div>
      </div>
                ...
                <div className="card-body p-0">
                    <table className="table">
                        ...
                        <tfoot className="bg-gray-50 font-medium">
                            ...
                            <tr><td colSpan="4" className="text-right">Impuesto ({parseFloat(compra.igv) > 0 ? '18%' : '0%'})</td><td className="text-right">{formatearMoneda(compra.igv)}</td></tr>
                            <tr><td colSpan="4" className="text-right font-bold text-lg">Total</td><td className="text-right font-bold text-lg">{formatearMoneda(compra.total)}</td></tr>
                        </tfoot>
                    </table>
                </div>
            </div>
                                ...
                                {compra.cuotas.map((cuota) => (
                                    <div key={cuota.id_cuota} className="p-3 hover:bg-gray-50 flex justify-between items-center">
                                        ...
                                        <div className="text-right">
                                            ...
                                            {cuota.estado === 'Pagada' ? <span className="badge badge-success text-[10px]">Pagada</span> : cuota.estado === 'Cancelada' ? <span className="badge badge-danger text-[10px]">Cancelada</span> : <button className="btn btn-xs btn-primary mt-1" onClick={() => handleAbrirPagarCuota(cuota)}>Pagar</button>}
                                        </div>
                                    </div>
                                ))}
                            </div>
        ...
        <div className="card">
          ...
          <div className="card-body p-0">
              {compra.pagos_realizados && compra.pagos_realizados.length > 0 ? (
                  <table className="table">
                      <thead><tr><th>Fecha</th><th>Tipo</th><th>Cuenta Origen</th><th>Concepto</th><th>Ref.</th><th className="text-right">Monto</th></tr></thead>
                      <tbody>
                          {compra.pagos_realizados.map((pago, i) => (
                              <tr key={i} className={pago.tipo_movimiento === 'Ingreso' ? 'bg-red-50' : ''}>
                                  ...
                                  <td>{pago.tipo_movimiento === 'Ingreso' ? <span className="badge badge-danger">Anulación</span> : pago.es_reembolso === 1 ? <span className="badge badge-purple flex items-center gap-1 w-fit"><RefreshCw size={12} /> Reembolso</span> : <span className="badge badge-info">Pago</span>}</td>
                                  <td><div className="font-medium">{pago.cuenta_origen}</div><div className="text-xs text-muted">{pago.tipo_cuenta}</div></td>
                                  <td className="text-sm">{pago.concepto || '-'}</td>
                                  <td className="text-sm">{pago.referencia || '-'}</td>
                                  <td className={`text-right font-bold ${pago.tipo_movimiento === 'Ingreso' ? 'text-danger' : 'text-success'}`}>{pago.tipo_movimiento === 'Ingreso' ? '-' : ''}{formatearMoneda(pago.monto)}</td>
                              </tr>
                          ))}
                      </tbody>
                  </table>
        ...
        <div className="space-y-6">
          <div className="card">
            ...
            <div className="card-body p-0">
              {letras.length > 0 ? (
                <table className="table">
                  <thead><tr><th>Número Letra</th><th>Banco</th><th>Emisión</th><th>Vencimiento</th><th>Estado</th><th className="text-right">Monto</th><th className="text-center">Acciones</th></tr></thead>
                  <tbody>
                    {letras.map((letra) => (
                      <tr key={letra.id_letra} className={letra.nivel_alerta === 'danger' ? 'bg-red-50' : letra.nivel_alerta === 'warning' ? 'bg-yellow-50' : ''}>
                        <td><div className="font-medium">{letra.numero_letra}</div>{letra.dias_para_vencer !== undefined && letra.dias_para_vencer < 7 && letra.estado === 'Pendiente' && <div className="text-xs text-warning">Vence en {letra.dias_para_vencer} días</div>}</td>
                        <td className="text-sm">{letra.banco || '-'}</td>
                        <td>{formatearFecha(letra.fecha_emision)}</td>
                        <td>{formatearFecha(letra.fecha_vencimiento)}</td>
                        <td><span className={`badge ${letra.estado === 'Pagada' ? 'badge-success' : letra.estado === 'Vencida' ? 'badge-danger' : 'badge-warning'}`}>{letra.estado}</span></td>
                        <td className="text-right font-bold">{formatearMoneda(letra.monto)}</td>
                        <td className="text-center">{letra.estado === 'Pendiente' && <button className="btn btn-xs btn-primary" onClick={() => handleAbrirPagarLetra(letra)}>Pagar</button>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="p-8 text-center text-muted">
                  ...
                  {!compra.letras_registradas && <button className="btn btn-sm btn-primary mt-3" onClick={handleAbrirRegistrarLetras}>Registrar Letras</button>}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          {itemsPendientes.length > 0 ? (
            <div className="card border-l-4 border-warning">
              <div className="card-body">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="font-bold text-warning flex items-center gap-2"><AlertCircle size={18} /> Items Pendientes de Ingreso</h3>
                    <p className="text-sm text-muted mt-1">Hay {itemsPendientes.length} producto(s) pendiente(s) de ingresar al inventario</p>
                  </div>
                  <button className="btn btn-primary" onClick={handleAbrirIngresoInventario}><PackagePlus size={18} /> Registrar Ingreso</button>
                </div>
              </div>
            </div>
          ) : (
          ...
          <div className="card">
            ...
            <div className="card-body p-0">
              {ingresos.length > 0 ? (
                <table className="table">
                  <thead><tr><th>Fecha</th><th>ID Entrada</th><th className="text-right">Items Ingresados</th><th className="text-right">Total</th><th>Estado</th><th>Registrado Por</th></tr></thead>
                  <tbody>
                    {ingresos.map((ingreso) => (
                      <tr key={ingreso.id_entrada}>
                        ...
                        <td><span className={`badge ${ingreso.estado_ingreso === 'Completo' ? 'badge-success' : 'badge-info'}`}>{ingreso.estado_ingreso}</span></td>
                        <td className="text-sm">{ingreso.registrado_por || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ) : <div className="p-8 text-center text-muted"><Truck size={48} className="mx-auto mb-3 opacity-20" /><p>No hay ingresos registrados</p></div>}
            </div>
          </div>
        </div>
      )}
            ...
            <div className="space-y-2 mb-4 max-h-[60vh] overflow-y-auto">
                {cronogramaForm.map((c, i) => (
                    <div key={i} className="flex gap-2 items-center">
                        ...
                        <input type="date" className="form-input w-32" value={c.fecha_vencimiento} onChange={e => { const newCrono = [...cronogramaForm]; newCrono[i].fecha_vencimiento = e.target.value; setCronogramaForm(newCrono); }} required />
                        <input type="number" className="form-input flex-1" placeholder="Monto" value={c.monto} onChange={e => { const newCrono = [...cronogramaForm]; newCrono[i].monto = e.target.value; setCronogramaForm(newCrono); }} step="0.01" required />
                        <input type="text" className="form-input flex-1" placeholder="Cód. Letra (Opcional)" value={c.codigo_letra} onChange={e => { const newCrono = [...cronogramaForm]; newCrono[i].codigo_letra = e.target.value; setCronogramaForm(newCrono); }} />
                    </div>
                ))}
            </div>
            <div className="flex justify-between items-center pt-4 border-t">
                <div className="text-sm">Total Letras: <b>{formatearMoneda(cronogramaForm.reduce((acc, c) => acc + parseFloat(c.monto||0), 0))}</b> <span className="text-muted ml-2">(Debe ser {formatearMoneda(compra.saldo_pendiente)})</span></div>
            ...
            {letrasForm.map((letra, i) => (
              <div key={i} className="border rounded p-3 bg-gray-50">
                <div className="grid grid-cols-2 gap-3">
                  <div className="form-group"><label className="form-label text-xs">Número Letra</label><input type="text" className="form-input form-input-sm" value={letra.numero_letra} onChange={e => { const nuevasLetras = [...letrasForm]; nuevasLetras[i].numero_letra = e.target.value; setLetrasForm(nuevasLetras); }} required /></div>
                  <div className="form-group"><label className="form-label text-xs">Monto</label><input type="number" className="form-input form-input-sm" value={letra.monto} onChange={e => { const nuevasLetras = [...letrasForm]; nuevasLetras[i].monto = e.target.value; setLetrasForm(nuevasLetras); }} step="0.01" required /></div>
                  <div className="form-group"><label className="form-label text-xs">Fecha Emisión</label><input type="date" className="form-input form-input-sm" value={letra.fecha_emision} onChange={e => { const nuevasLetras = [...letrasForm]; nuevasLetras[i].fecha_emision = e.target.value; setLetrasForm(nuevasLetras); }} required /></div>
                  <div className="form-group"><label className="form-label text-xs">Fecha Vencimiento</label><input type="date" className="form-input form-input-sm" value={letra.fecha_vencimiento} onChange={e => { const nuevasLetras = [...letrasForm]; nuevasLetras[i].fecha_vencimiento = e.target.value; setLetrasForm(nuevasLetras); }} required /></div>
                  <div className="form-group col-span-2"><label className="form-label text-xs">Banco</label><input type="text" className="form-input form-input-sm" value={letra.banco} onChange={e => { const nuevasLetras = [...letrasForm]; nuevasLetras[i].banco = e.target.value; setLetrasForm(nuevasLetras); }} placeholder="Opcional" /></div>
                </div>
              </div>
            ))}
          </div>
          <div className="flex justify-between items-center pt-4 border-t">
            <div className="text-sm">Total: <b>{formatearMoneda(letrasForm.reduce((acc, l) => acc + parseFloat(l.monto||0), 0))}</b></div>
            <div className="flex gap-2">
              <button type="button" className="btn btn-outline" onClick={() => { setLetrasForm([...letrasForm, { numero_letra: `L-${compra.numero_orden}-${String(letrasForm.length + 1).padStart(2, '0')}`, monto: '0.00', fecha_emision: new Date().toISOString().split('T')[0], fecha_vencimiento: new Date().toISOString().split('T')[0], banco: '', observaciones: '' }]); }}><Plus size={16} /> Agregar Letra</button>
      ...
      <Modal isOpen={modalPagarLetraOpen} onClose={() => setModalPagarLetraOpen(false)} title="Pagar Letra">
        <form onSubmit={handlePagarLetra} className="space-y-4">
          ...
          <div className="form-group"><label className="form-label">Monto</label><input type="number" className="form-input" value={datosLetra.monto_pagado} onChange={e => setDatosLetra({...datosLetra, monto_pagado: e.target.value})} step="0.01" required /></div>
          <div className="form-group">
          ...
          <div className="form-group"><label className="form-label">Número Operación</label><input type="text" className="form-input" value={datosLetra.numero_operacion} onChange={e => setDatosLetra({...datosLetra, numero_operacion: e.target.value})} /></div>
          <button type="submit" className="btn btn-primary w-full" disabled={loading}>Confirmar Pago</button>
        </form>
      </Modal>
      ...
      <Modal isOpen={modalIngresoInventarioOpen} onClose={() => setModalIngresoInventarioOpen(false)} title="Registrar Ingreso a Inventario" size="lg">
        <form onSubmit={handleRegistrarIngreso}>
          <div className="space-y-3 mb-4 max-h-[60vh] overflow-y-auto">
            <table className="table">
              <thead><tr><th>Producto</th><th className="text-right">Pendiente</th><th className="text-right w-32">Ingresar</th></tr></thead>
              <tbody>
                {ingresoForm.productos.map((prod) => (
                  <tr key={prod.id_producto}>
                    <td><div className="font-medium">{prod.nombre}</div><div className="text-xs text-muted">{prod.codigo}</div></td>
                    <td className="text-right text-warning font-medium">{prod.cantidad_pendiente.toFixed(2)}</td>
                    <td><input type="number" className="form-input text-right" value={prod.cantidad_ingresar} onChange={e => handleActualizarCantidadIngreso(prod.id_producto, e.target.value)} min="0" max={prod.cantidad_pendiente} step="0.01" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <div className="form-group"><label className="form-label">Observaciones</label><textarea className="form-textarea" rows="2" value={ingresoForm.observaciones} onChange={e => setIngresoForm({...ingresoForm, observaciones: e.target.value})} placeholder="Notas sobre el ingreso..." /></div>
          <div className="flex justify-end gap-2 pt-4 border-t">
            <button type="button" className="btn btn-outline" onClick={() => setModalIngresoInventarioOpen(false)}>Cancelar</button>
            <button type="submit" className="btn btn-primary" disabled={loading}><PackagePlus size={18} /> Registrar Ingreso</button>
          </div>
        </form>
      </Modal>
      ...
      <Modal isOpen={modalCancelarOpen} onClose={() => setModalCancelarOpen(false)} title="Cancelar Compra y Anular Movimientos">
        <div className="space-y-4">
            ...
            <div className="form-group">
              ...
              <textarea className="form-textarea" placeholder="Indique la razón..." value={motivoCancelacion} onChange={e => setMotivoCancelacion(e.target.value)} required rows="3" />
            </div>
      ...
      <Modal isOpen={modalCambiarCuentaOpen} onClose={() => setModalCambiarCuentaOpen(false)} title="Cambiar Cuenta de Pago (Migración)">
        <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded p-4">
                <p className="text-sm text-blue-800">Esta acción transferirá todos los pagos y la deuda de esta compra a una nueva cuenta. El dinero pagado será "devuelto" a la cuenta actual y "descontado" de la nueva.</p>
            </div>
            <div className="form-group">
