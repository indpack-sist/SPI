...
function NuevaCompra() {
  ...
  useEffect(() => {
    if (formData.tipo_compra === 'Contado') {
        setFormData(prev => ({ ...prev, monto_pagado_inicial: totalsRef.current })); 
    }
  }, [totales.total, formData.tipo_compra]);

  const totalsRef = { current: 0 };
  totalsRef.current = totales.total;

  useEffect(() => {
    if (formData.tipo_compra === 'Credito') {
      calcularCronograma();
    }
  }, [formData.tipo_compra, formData.numero_cuotas, formData.dias_entre_cuotas, formData.fecha_emision, formData.fecha_primera_cuota, totales.total]);
  ...
  const handleVerHistorial = async (producto) => {
    try {
      setLoadingHistorial(true); setProductoHistorial(producto); setModalHistorialOpen(true);
      const response = await productosAPI.getHistorialCompras(producto.id_producto, { limite: 20 });
      if (response.data.success) setHistorialCompras(response.data.data);
    } catch (err) { setError('Error al cargar el historial'); } finally { setLoadingHistorial(false); }
  };
  ...
  const calcularMontoConversion = () => {
    ...
    if (cuentaSeleccionada.moneda === 'PEN' && formData.moneda === 'USD') return total * tc;
    else if (cuentaSeleccionada.moneda === 'USD' && formData.moneda === 'PEN') return total / tc;
    return null;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.monto_pagado_inicial > 0 && !formData.id_cuenta_pago) { setError('Seleccione cuenta para el pago inicial'); return; }
    ...
    if (formData.monto_pagado_inicial > 0 && cuentaSeleccionada.tipo === 'Tarjeta') {
        ...
        if (parseFloat(cuentaSeleccionada.saldo_actual) < montoRequerido) {
            setError(`Cupo insuficiente en tarjeta. Requerido: ${formatearMoneda(montoRequerido, cuentaSeleccionada.moneda)}`); return;
        }
    }
    ...
    try {
      ...
      const payload = {
        ...
        monto_pagado_inicial: parseFloat(formData.monto_pagado_inicial || 0),
